...
function makeball(x,y,radius,speed,color) {
    if (!color) color = '#000000';
    return {
        x: x,
        y: y,
        ...
        draw: function () {
            context.beginPath();
            context.arc(this.x, this.y, this.h, 0, 2 * Math.PI, false);
            context.fillStyle = this.c;
            context.fill();
            context.lineWidth = 5;
...
var leftPaddle = makeRect(25, canvas.height / 2 - paddleHeight / 2, paddleWidth, paddleHeight, 8, '#BC0000');
...
var ballLength = 7;
var ballSpeed = 2;
var ball = makeball(canvas.width / 2, canvas.height / 2, ballLength, ballSpeed, '#000000');
...
function resetBall() {
    ball.x = canvas.width / 2 - ball.h / 2;
